...
Сверх обязательного `MERGE` в проекте реализованы и проверены:

- `OPTIMIZE` - склеивает много мелких parquet-файлов в меньшее число крупных файлов, снижая overhead на открытие файлов, метаданные и планирование чтения;
- `Z-ORDER` - переупорядочивает строки так, чтобы близкие значения колонок вроде source_date, origin, dest лежали рядом, и фильтры по ним читали меньше данных
- `VACUUM` - убирает старые уже неиспользуемые файлы после переписываний таблицы и освобождает место
- `time travel` - аудит, можно прочитать старую версию таблицы

Фактический результат проверки на `bronze`:

- до оптимизации: `version=30`, `31` parquet-файл;
- после `OPTIMIZE` compaction: `version=31`, `1` parquet-файл;
- после `Z-ORDER` по `source_date, origin, dest`: `version=32`;
- `time travel` к версии `30` успешно прочитал исторический снимок на `582425` строк;
- `VACUUM` в `dry-run` режиме нашел `32` устаревших файла-кандидата к удалению.
